...
|---|---|---|
| 面板查看 | ✅ | 时序、表格、仪表盘聚合卡片 |
| PromQL 查询 | ✅ | 与 Prometheus 兼容核心查询链路，补齐 `labels/label values/metadata/status/targets` 等 Grafana 常用探测端点 |
...
- [ ] Grafana 变量查询已通过 `/api/v1/label/{name}/values` 或 `/api/v1/labels` 验证，`query_range` 的 RFC3339 时间和 duration step 已覆盖
