...
datasources:
  - name: Prometheus
    ...
    isDefault: true
'@
  $ds = Invoke-JsonApi -Method "POST" -Url "$BaseUrl/api/compat/import?source=grafana-datasources" -Headers ($headers + @{ "Content-Type" = "application/yaml" }) -Body $datasources
...
'@
  $targets = Invoke-JsonApi -Method "POST" -Url "$BaseUrl/api/compat/import?source=prometheus-config" -Headers ($headers + @{ "Content-Type" = "application/yaml" }) -Body $promConfig
    ...
    rules:
      - alert: CheckoutTrafficMissing
        ...
        labels:
          severity: warning
'@
  $rules = Invoke-JsonApi -Method "POST" -Url "$BaseUrl/api/compat/import?source=prometheus-rules" -Headers ($headers + @{ "Content-Type" = "application/yaml" }) -Body $rulesYaml
...
'@
  ...
  $webhook = Invoke-JsonApi -Method "POST" -Url "$BaseUrl/api/compat/alertmanager/webhook" -Body $alertPayload
  ...
  Write-Host "Docker Grafana replacement E2E passed for $BaseUrl"
} finally {
  if (-not $KeepRunning) {
