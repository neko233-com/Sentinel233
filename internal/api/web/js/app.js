...
const pageMeta = {
  ...
  integrations: ['接入中心', '接入主流 Prometheus exporter、数据库、中间件、云原生和 Sentinel 高性能客户端'],
...
const dashboardPresets = [
  ...
  {
    title: 'Elasticsearch 集群总览',
    description: 'elasticsearch_exporter 的集群健康、节点、JVM、索引、分片和线程池指标。',
    panels: [
      { title: '集群健康', type: 'stat', query: 'elasticsearch_cluster_health_status' },
      { title: '节点在线', type: 'stat', query: 'elasticsearch_cluster_health_number_of_nodes' },
      { title: 'JVM Heap 使用率', type: 'gauge', query: 'elasticsearch_jvm_memory_used_bytes{area="heap"} / elasticsearch_jvm_memory_max_bytes{area="heap"} * 100', unit: 'percent' },
      { title: '索引文档数', type: 'timeseries', query: 'sum(elasticsearch_indices_docs)' },
      { title: '搜索请求速率', type: 'timeseries', query: 'sum(rate(elasticsearch_indices_search_query_total[$__rate_interval])) by (cluster)' },
      { title: '线程池拒绝', type: 'table', query: 'rate(elasticsearch_thread_pool_rejected_count[$__rate_interval])' },
    ],
  },
  {
    title: 'MongoDB 运行总览',
    description: 'mongodb_exporter 的连接、操作速率、锁、复制和 WiredTiger 存储指标。',
    panels: [
      { title: 'MongoDB 在线', type: 'stat', query: 'mongodb_up' },
      { title: '连接数', type: 'gauge', query: 'mongodb_connections{state="current"}' },
      { title: '操作速率', type: 'timeseries', query: 'sum(rate(mongodb_op_counters_total[$__rate_interval])) by (type)' },
      { title: '复制延迟', type: 'timeseries', query: 'mongodb_mongod_replset_member_replication_lag' },
      { title: 'Page Faults', type: 'timeseries', query: 'rate(mongodb_extra_info_page_faults_total[$__rate_interval])' },
      { title: '集合与数据库', type: 'table', query: 'mongodb_ss_dbs_collections' },
    ],
  },
  {
    title: 'Kafka 集群总览',
    description: 'kafka_exporter 与 JMX exporter 的 broker、topic、partition、consumer lag 和 ISR 指标。',
    panels: [
      { title: 'Brokers', type: 'stat', query: 'count(kafka_brokers)' },
      { title: 'Consumer Lag', type: 'timeseries', query: 'sum(kafka_consumergroup_lag) by (consumergroup, topic)' },
      { title: 'Under Replicated Partitions', type: 'stat', query: 'sum(kafka_topic_partition_under_replicated_partition)' },
      { title: '消息流入', type: 'timeseries', query: 'sum(rate(kafka_server_brokertopicmetrics_messagesin_total[$__rate_interval])) by (topic)' },
      { title: 'Bytes In', type: 'timeseries', query: 'sum(rate(kafka_server_brokertopicmetrics_bytesin_total[$__rate_interval])) by (topic)', unit: 'bytes' },
      { title: 'Offline Partitions', type: 'table', query: 'kafka_controller_kafkacontroller_offlinepartitionscount' },
    ],
  },
  {
    title: 'RabbitMQ 运行总览',
    description: 'rabbitmq_exporter 的队列、连接、消息吞吐、消费者和节点健康指标。',
    panels: [
      { title: 'RabbitMQ 在线', type: 'stat', query: 'rabbitmq_up' },
      { title: 'Ready 消息', type: 'timeseries', query: 'sum(rabbitmq_queue_messages_ready) by (queue)' },
      { title: 'Unacked 消息', type: 'timeseries', query: 'sum(rabbitmq_queue_messages_unacked) by (queue)' },
      { title: '发布速率', type: 'timeseries', query: 'sum(rate(rabbitmq_queue_messages_published_total[$__rate_interval])) by (queue)' },
      { title: '消费速率', type: 'timeseries', query: 'sum(rate(rabbitmq_queue_messages_delivered_total[$__rate_interval])) by (queue)' },
      { title: '消费者', type: 'table', query: 'rabbitmq_queue_consumers' },
    ],
  },
  {
    title: 'HAProxy 入口总览',
    description: 'haproxy_exporter 的 frontend/backend 会话、响应码、错误和队列指标。',
    panels: [
      { title: 'HAProxy 在线', type: 'stat', query: 'haproxy_up' },
      { title: '当前会话', type: 'timeseries', query: 'sum(haproxy_frontend_current_sessions) by (proxy)' },
      { title: '请求速率', type: 'timeseries', query: 'sum(rate(haproxy_frontend_http_requests_total[$__rate_interval])) by (proxy)' },
      { title: '5xx 速率', type: 'timeseries', query: 'sum(rate(haproxy_backend_http_responses_total{code="5xx"}[$__rate_interval])) by (proxy)' },
      { title: '后端队列', type: 'table', query: 'haproxy_backend_current_queue' },
      { title: '后端健康', type: 'table', query: 'haproxy_server_up' },
    ],
  },
  {
    title: 'etcd 集群总览',
    description: 'etcd 内置 Prometheus 指标的 leader、提案、磁盘、网络和 DB size。',
    panels: [
      { title: 'Leader', type: 'stat', query: 'sum(etcd_server_has_leader)' },
      { title: 'Leader Changes', type: 'timeseries', query: 'rate(etcd_server_leader_changes_seen_total[$__rate_interval])' },
      { title: '提案失败', type: 'timeseries', query: 'rate(etcd_server_proposals_failed_total[$__rate_interval])' },
      { title: 'Commit 延迟 P99', type: 'timeseries', query: 'histogram_quantile(0.99, sum(rate(etcd_disk_backend_commit_duration_seconds_bucket[$__rate_interval])) by (le))' },
      { title: 'DB Size', type: 'timeseries', query: 'etcd_mvcc_db_total_size_in_bytes', unit: 'bytes' },
      { title: 'Peer 流量', type: 'timeseries', query: 'rate(etcd_network_peer_sent_bytes_total[$__rate_interval])', unit: 'bytes' },
    ],
  },
  {
    title: 'MinIO 对象存储总览',
    description: 'MinIO Prometheus 指标的容量、请求、错误、磁盘和 bucket 视图。',
    panels: [
      { title: '节点在线', type: 'stat', query: 'up{job=~"minio|miniocluster"}' },
      { title: '磁盘使用', type: 'timeseries', query: 'minio_node_drive_used_bytes', unit: 'bytes' },
      { title: '可用容量', type: 'timeseries', query: 'minio_cluster_capacity_usable_free_bytes', unit: 'bytes' },
      { title: '请求速率', type: 'timeseries', query: 'sum(rate(minio_s3_requests_total[$__rate_interval])) by (api)' },
      { title: '错误速率', type: 'timeseries', query: 'sum(rate(minio_s3_errors_total[$__rate_interval])) by (api)' },
      { title: 'Bucket 对象数', type: 'table', query: 'minio_bucket_objects_count' },
    ],
  },
  {
    title: 'Windows Exporter 总览',
    description: 'windows_exporter 的 CPU、内存、磁盘、网络、服务和系统指标。',
    panels: [
      { title: 'Windows 在线', type: 'stat', query: 'up{job=~"windows|windows_exporter"}' },
      { title: 'CPU 使用率', type: 'timeseries', query: '100 - (avg by (instance) (rate(windows_cpu_time_total{mode="idle"}[$__rate_interval])) * 100)', unit: 'percent' },
      { title: '内存使用', type: 'timeseries', query: 'windows_os_physical_memory_free_bytes', unit: 'bytes' },
      { title: '磁盘空闲', type: 'bar', query: 'windows_logical_disk_free_bytes', unit: 'bytes' },
      { title: '网络接收', type: 'timeseries', query: 'rate(windows_net_bytes_received_total[$__rate_interval])', unit: 'bytes' },
      { title: '服务状态', type: 'table', query: 'windows_service_state' },
    ],
  },
  {
    title: 'SNMP 网络设备总览',
    description: 'snmp_exporter 的接口流量、错误、丢包、设备在线和容量指标。',
    panels: [
      { title: '设备在线', type: 'stat', query: 'up{job=~"snmp|network"}' },
      { title: '接口入流量', type: 'timeseries', query: 'rate(ifHCInOctets[$__rate_interval])', unit: 'bytes' },
      { title: '接口出流量', type: 'timeseries', query: 'rate(ifHCOutOctets[$__rate_interval])', unit: 'bytes' },
      { title: '入错误', type: 'timeseries', query: 'rate(ifInErrors[$__rate_interval])' },
      { title: '出错误', type: 'timeseries', query: 'rate(ifOutErrors[$__rate_interval])' },
      { title: '接口状态', type: 'table', query: 'ifOperStatus' },
    ],
  },
  {
    title: 'OpenTelemetry Collector 总览',
    description: 'otelcol 自身的 receiver、processor、exporter、队列和丢弃指标。',
    panels: [
      { title: 'Collector 在线', type: 'stat', query: 'up{job=~"otelcol|otel-collector"}' },
      { title: '接收 Metric Points', type: 'timeseries', query: 'sum(rate(otelcol_receiver_accepted_metric_points[$__rate_interval])) by (receiver)' },
      { title: '拒绝 Metric Points', type: 'timeseries', query: 'sum(rate(otelcol_receiver_refused_metric_points[$__rate_interval])) by (receiver)' },
      { title: 'Exporter 发送', type: 'timeseries', query: 'sum(rate(otelcol_exporter_sent_metric_points[$__rate_interval])) by (exporter)' },
      { title: 'Exporter 失败', type: 'timeseries', query: 'sum(rate(otelcol_exporter_send_failed_metric_points[$__rate_interval])) by (exporter)' },
      { title: '队列容量', type: 'table', query: 'otelcol_exporter_queue_capacity' },
    ],
  },
];
...
const integrationPresets = [
  ...
  },
  {
    ...
    kind: 'Search exporter',
    endpoint: 'http://localhost:9114/metrics',
    labels: { job: 'elasticsearch', search: 'elasticsearch' },
    summary: '接入 Elasticsearch 集群健康、节点、JVM、索引、分片、线程池和查询压力。',
    snippet: `./elasticsearch_exporter \\\n  --es.uri=http://127.0.0.1:9200 \\\n  --web.listen-address=:9114`,
    dashboards: ['Elasticsearch 集群总览', '存储与基数巡检'],
  },
  {
    id: 'mongodb-exporter',
    title: 'MongoDB / mongodb_exporter',
    kind: 'Database exporter',
    endpoint: 'http://localhost:9216/metrics',
    labels: { job: 'mongodb', db: 'mongodb' },
    summary: '接入 MongoDB 连接、操作吞吐、锁、复制延迟、集合和 WiredTiger 存储状态。',
    snippet: `export MONGODB_URI='mongodb://exporter:password@127.0.0.1:27017/admin'\n./mongodb_exporter --web.listen-address=:9216`,
    dashboards: ['MongoDB 运行总览', '存储与基数巡检'],
  },
  {
    id: 'kafka-exporter',
    title: 'Kafka / kafka_exporter + JMX',
    kind: 'Queue exporter',
    endpoint: 'http://localhost:9308/metrics',
    labels: { job: 'kafka', queue: 'kafka' },
    summary: '接入 Kafka broker、topic、partition、consumer lag、ISR、controller 和 JMX 运行时指标。',
    snippet: `./kafka_exporter \\\n  --kafka.server=127.0.0.1:9092 \\\n  --web.listen-address=:9308\n\n# JVM broker metrics can also come from jmx_exporter.`,
    dashboards: ['Kafka 集群总览', 'JVM 服务运行总览'],
  },
  {
    id: 'rabbitmq-exporter',
    title: 'RabbitMQ / rabbitmq_exporter',
    kind: 'Queue exporter',
    endpoint: 'http://localhost:9419/metrics',
    labels: { job: 'rabbitmq', queue: 'rabbitmq' },
    summary: '接入 RabbitMQ 队列深度、连接、channel、消费者、消息发布/投递速率和节点健康。',
    snippet: `export RABBIT_URL='http://127.0.0.1:15672'\nexport RABBIT_USER='exporter'\nexport RABBIT_PASSWORD='password'\n./rabbitmq_exporter --web.listen-address=:9419`,
    dashboards: ['RabbitMQ 运行总览'],
  },
  {
    id: 'haproxy-exporter',
    title: 'HAProxy / haproxy_exporter',
    kind: 'Edge exporter',
    endpoint: 'http://localhost:9101/metrics',
    labels: { job: 'haproxy', edge: 'haproxy' },
    summary: '接入 HAProxy frontend/backend 会话、响应码、错误、队列和后端健康。',
    snippet: `./haproxy_exporter \\\n  --haproxy.scrape-uri=http://127.0.0.1:8404/metrics \\\n  --web.listen-address=:9101`,
    dashboards: ['HAProxy 入口总览', 'Web 服务 SLO'],
  },
  {
    id: 'etcd',
    title: 'etcd / built-in metrics',
    kind: 'Cluster metrics',
    endpoint: 'http://localhost:2379/metrics',
    labels: { job: 'etcd', platform: 'etcd' },
    summary: '接入 etcd leader、提案、磁盘 fsync、网络 peer、DB size 和 raft 健康。',
    snippet: `# etcd exposes Prometheus metrics on the client endpoint by default\ncurl http://127.0.0.1:2379/metrics`,
    dashboards: ['etcd 集群总览', '存储与基数巡检'],
  },
  {
    id: 'minio',
    title: 'MinIO / built-in Prometheus metrics',
    kind: 'Storage metrics',
    endpoint: 'http://localhost:9000/minio/v2/metrics/cluster',
    labels: { job: 'minio', storage: 'object' },
    summary: '接入 MinIO 集群容量、磁盘、S3 请求、错误、bucket 对象数和节点状态。',
    snippet: `mc admin prometheus generate local\n\n# scrape: http://minio:9000/minio/v2/metrics/cluster`,
    dashboards: ['MinIO 对象存储总览', '存储与基数巡检'],
  },
  {
    id: 'windows-exporter',
    title: 'Windows / windows_exporter',
    kind: 'Host exporter',
    endpoint: 'http://localhost:9182/metrics',
    labels: { job: 'windows', os: 'windows' },
    summary: '接入 Windows CPU、内存、磁盘、网络、服务、进程和系统状态。',
    snippet: `msiexec /i windows_exporter.msi ENABLED_COLLECTORS='cpu,cs,logical_disk,net,os,service,system,textfile'`,
    dashboards: ['Windows Exporter 总览', '主机与进程基础盘'],
  },
  {
    id: 'snmp-exporter',
    title: 'Network / snmp_exporter',
    kind: 'Network exporter',
    endpoint: 'http://localhost:9116/snmp?target=192.0.2.10&module=if_mib',
    labels: { job: 'snmp', network: 'device' },
    summary: '接入交换机、路由器、防火墙、UPS 等 SNMP 设备的接口流量、错误、丢包和状态。',
    snippet: `./snmp_exporter --web.listen-address=:9116\n\n# scrape /snmp?target=<device-ip>&module=if_mib`,
    dashboards: ['SNMP 网络设备总览'],
  },
  {
    id: 'otel-collector',
    title: 'OpenTelemetry Collector',
    kind: 'Pipeline metrics',
    endpoint: 'http://localhost:8888/metrics',
    labels: { job: 'otelcol', telemetry: 'opentelemetry' },
    summary: '接入 OTel Collector 自身 receiver、processor、exporter、队列、拒绝和发送失败指标。',
    snippet: `service:\n  telemetry:\n    metrics:\n      address: 0.0.0.0:8888`,
    dashboards: ['OpenTelemetry Collector 总览'],
  },
  {
    id: 'sentinel-lib-go',
    ...
    <div class="workspace split">
      <section class="band">
        ...
        <div class="doc-list">
          ...
          <div class="doc-item"><strong>3. 从接入中心开始</strong><p>内置 Go/JVM、Linux/Windows、MySQL、PostgreSQL、Redis、Elasticsearch、MongoDB、Kafka、RabbitMQ、Nginx、HAProxy、Docker/cAdvisor、Kubernetes、etcd、MinIO、SNMP、Blackbox、OpenTelemetry Collector 和 Sentinel233 Go client lib 的接入模板。</p></div>
